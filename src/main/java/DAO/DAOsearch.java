...
public class DAOsearch extends DAOcrud {
	...
	public ArrayList<DTOjoin> listSearchReply(String sort, String currentPage, String keyword) {
		...
		try {
			...
			while (rs.next()) {
				DTOjoin dtoJ = new DTOjoin(
						rs.getString(DB.SERVER_BOARD+".fm_num"), rs.getString(DB.SERVER_BOARD+".fm_title"), rs.getString(DB.SERVER_BOARD+".fm_point"), rs.getString(DB.SERVER_BOARD+".fm_reply"),
