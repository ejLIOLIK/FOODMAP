...
<%}
	...
	<a href="/board/rightWriteR_login?category=<%=request.getParameter("category")%>&postNum=<%=d.Pnum%>">수정</a>	 / 
	<a href="/board/rightWriteR_login?category=<%=request.getParameter("category")%>&postNum=<%=DB.dto.num%>">삭제</a>
	<%}
	else if(checkID.equals(d.id)){%>
	<a href="/board/read?category=<%=request.getParameter("category")%>&editReplyNum=<%=d.Rnum%>&postNum=<%=d.Pnum%>&currentPage=<%=request.getParameter("currentPage")%>&sort=<%=request.getParameter("sort")%>&keyword=<%=request.getParameter("keyword")%>">수정</a>	 / 
	<a href="/board/replyDelete?category=<%=request.getParameter("category")%>&delNum=<%=d.Rnum%>&postNum=<%=DB.dto.num%>&currentPage=<%=request.getParameter("currentPage")%>&sort=<%=request.getParameter("sort")%>&keyword=<%=request.getParameter("keyword")%>">삭제</a>
	<%}
	else{%> 
	<a href="/board/rightWriteR_id?category=<%=request.getParameter("category")%>&postNum=<%=d.Pnum%>">수정</a>	 / 
	<a href="/board/rightWriteR_id?category=<%=request.getParameter("category")%>&postNum=<%=DB.dto.num%>">삭제</a>
	<%}%>
	<br>
	<%
} countReply++; }
